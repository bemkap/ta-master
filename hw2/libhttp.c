...
int http_send_file(int fd,char*file_name){
  char buffer[1024];
  FILE*fin;
  size_t n;
  struct stat st;
  stat(file_name,&st);
  if(NULL==(fin=fopen(file_name,"rb"))) return -1;
  http_start_response(fd,200);
  http_send_header(fd,"Content-type",http_get_mime_type(file_name));
  dprintf(fd,"Content-length: %d\r\n",(size_t)st.st_size);
  http_end_headers(fd);
  while(0<(n=fread(buffer,1,sizeof(buffer),fin)))
    http_send_data(fd,buffer,n);
  fclose(fin);
  return 0;
}

void http_send_anchor(int fd,char*data){
  http_send_string(fd,"<p><a href=");
  http_send_string(fd,data);
  http_send_string(fd,">");
  http_send_string(fd,data);
  http_send_string(fd,"</a></p>");
}
